...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Rozhnov Denis P3400\nTarasenko Daniil P3400");
MODULE_DESCRIPTION("Lab 1 Variant 5");
MODULE_VERSION("0.1");
	
static int custom_open(struct inode*, struct file*);
static int custom_close(struct inode*, struct file*);
...
static ssize_t custom_write(struct file *f, const char __user *ubuf, size_t len, loff_t *off)
{
	char buf[BUFSIZE];
